...
export default class App {
  constructor() {
    ...
    this.desktopPath = DesktopPathResolver.resolveDesktopPath(
      this.userDataDir,
      this.l
    );
    this.loadSettings();
    ...
    if (
      'builderOptions' in this.settings &&
      this.settings.builderOptions.appId
    ) {
    ...
    if (this.isProduction() && !this.settings.prodDebug) {
      ...
      require('electron-debug')({
        ...
        enabled:
          this.settings.devTools !== undefined ? this.settings.devTools : true,
      });
    }
    ...
    this.eventsBus.on('newVersionReady', (version, desktopVersion) => {
      this.l.debug(
        `received newVersionReady ${
          desktopVersion ? '(desktop update present)' : ''
        }`
      );
    ...
    this.eventsBus.on(
      'startupDidComplete',
      this.handleAppStartup.bind(this, true)
    );
    this.eventsBus.on('revertVersionReady', () => {
      this.meteorAppVersionChange = true;
    });
  ...
  applySingleInstance() {
    if ('singleInstance' in this.settings && this.settings.singleInstance) {
      ...
      if (isSecondInstance) {
        this.l.warn(
          'current instance was terminated because another instance is running'
        );
        app.quit();
      }
    }
  ...
  loadSettings() {
    try {
      this.settings = JSON.parse(
        fs.readFileSync(join(this.desktopPath, 'settings.json')),
        'UTF-8'
      );
    } catch (e) {
      this.l.error(e);
      dialog.showErrorBox(
        'Application',
        'Could not read settings.json. Please reinstall' + ' this application.'
      );
  ...
  uncaughtExceptionHandler() {
    ...
    setTimeout(() => {
      dialog.showErrorBox(
        'Application',
        'Internal error occurred. Restart this ' +
      ...
      Object.keys(this.settings.plugins).forEach((plugin) => {
        try {
          ...
          this.modules[plugin] = new Plugin({
            ...
            settings:
              typeof this.settings.plugins[plugin] === 'object'
  ...
  loadModules() {
    ...
    try {
      ...
      if (err.code === 'ENOENT') {
        this.l.debug(
          `not loading custom app modules because .desktop/modules isn't a directory`
        );
      } else {
        throw err;
      }
    ...
    moduleDirectories.forEach((dirName) => {
      try {
        ...
        this.l.error(
          `error while trying to load module in dir ${dirName}: ${e}`
        );
  ...
  static readModuleConfiguration(modulePath) {
    ...
    if ('extract' in moduleJson) {
      settings.extractedFilesPath = join(
        __dirname,
        '..',
        'extracted',
        moduleName
  ...
  onStartupFailed(code) {
    ...
    dialog.showErrorBox(
      'Startup error',
      'Could not initialize app. Please contact' +
  ...
  onServerReady(port) {
    ...
    this.webContents.on('will-navigate', (event, url) => {
      if (this.meteorAppVersionChange) {
        this.l.debug(
          `will-navigate event to ${url}, assuming that this is HCP refresh`
        );
        // We need to block it.
      ...
      (request, callback) => {
        ...
        this.modules.localServer
          ...
          .catch((e) => {
            ...
            this.log.error(
              `error while trying to fetch ${url}: ${e.toString()}`
            );
          });
      },
      (e) => {
        if (e) {
          this.l.error(
            `error while registering meteor:// protocol: ${e.toString()}`
          );
  ...
  handleAppStartup(startupDidCompleteEvent) {
    ...
    if (!this.windowAlreadyLoaded) {
      if (this.meteorAppVersionChange) {
        this.l.verbose(
          'there is a new version downloaded already, performing HCP' + ' reset'
        );
        this.updateToNewVersion();
      } else {
  ...
  updateToNewVersion() {
    ...
    const desktopUpdate =
      this.settings.desktopHCP &&
      this.settings.desktopVersion !== this.pendingDesktopVersion;

    this.emit(
      'beforeReload',
      this.modules.autoupdate.getPendingVersion(),
      desktopUpdate
    );
